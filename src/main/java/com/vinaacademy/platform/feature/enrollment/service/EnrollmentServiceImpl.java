...
public class EnrollmentServiceImpl implements EnrollmentService {
    ...
    public EnrollmentResponse enrollCourse(EnrollmentRequest request, UUID userId) {
        ...
        // Sử dụng UserClient để lấy thông tin người dùng từ service khác
        ...
        if (course.getPrice().longValue() != 0) {
            ...
            Optional<Order> order = orderRepository.findFirstByUserIdAndOrderItems_Course_IdAndStatusOrderByCreatedDateAsc(userId, course.getId(),
                    OrderStatus.PAID);
        ...
        Enrollment enrollment = Enrollment.builder()
                .userId(user.getId()).course(course).progressPercentage(0.0)
    ...
    public boolean isCourseOwnerByInstructor(UUID courseId, UUID instructorId) {
        ...
        // không
        return course.getInstructors().stream().anyMatch(instructor -> instructor.getInstructorId().equals(instructorId));
    }
    ...
    public PaginationResponse<StudentProgressDto> getStudentsProgressForInstructor(UUID instructorId, UUID courseId,
        ...
        if (courseId != null) {
            ...
            if (!isCourseOwnerByInstructor(courseId, instructorId)) {
                throw BadRequestException.message("Bạn không có quyền xem danh sách học viên của khóa học này");
            }
